...
Status: current as of 2026-01-24

This project completed incremental TypeScript migration with `allowJs: false` for `src/`.
Playwright tests in `tests/` are still JavaScript for now.
...
### New Files
- Always use `.ts` or `.tsx` extensions in `src/`
...
- `playwright.config.ts` - Playwright config (local)
