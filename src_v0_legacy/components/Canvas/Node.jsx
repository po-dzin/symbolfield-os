...
const Node = ({ node, isEditMode = false, scale = 1, onClick, onRightClick, onSourceOnboarding }) => {
    ...
    // Resolve Glyph
    const glyphChar = components.glyph?.char || getGlyphChar(components.glyph?.id);
    ...
    const selectionBorderColor = isLuma ? 'rgba(42, 38, 32, 0.25)' : 'rgba(255, 255, 255, 0.2)';
    const selectionBorderStrong = isLuma ? 'rgba(42, 38, 32, 0.45)' : 'rgba(255, 255, 255, 0.65)';
    const glyphColor = isLuma ? '#5b5349' : '#FFFFFF';
    ...
    const handleClick = (e) => {
        ...
        if (existingWindow?.isOpen && existingWindow?.data?.id === node.id) {
            ...
            // Preserve current position when switching active node
            openWindow(windowId, {
                title: 'PROPERTIES',
    ...
    const lumaDarken = isLuma ? (isWindowOpen ? 0.65 : 0.75) : 1;
    const lumaGlowFactor = isLuma ? (isWindowOpen ? 0.45 : 0.6) : 1;
    const coreGlowAlpha = isLuma ? 0.12 : 0.4;
    ...
    const baseGlowIntensity = isEditMode ? (isLuma ? 0.6 : 1.0) : 0.65;
    ...
    const shouldMaterialize = isCore && (now - (state?.materializedAt || 0) < 2000);

    // 2.71D Glassy Liquid Pointcloud Crystal Style (MORE BRIGHT)
    const crystalStyle = {
        background: isLuma
            ? `radial-gradient(circle at 35% 35%,
                rgba(225, 218, 206, ${0.65 * brightnessFactor * lumaDarken}) 0%,
                rgba(214, 206, 193, ${0.48 * brightnessFactor * lumaDarken}) 28%,
                rgba(${accentRGB}, ${0.1 * brightnessFactor * lumaDarken}) 55%,
                rgba(200, 190, 176, 0.5) 100%)`
            : `radial-gradient(circle at 35% 35%, 
        ...
        boxShadow: isLuma
            ? `
                inset -6px -6px 16px rgba(120, 110, 95, 0.16),
                inset 4px 4px 12px rgba(255,255,255,${0.32 * brightnessFactor * lumaDarken}),
                0 0 ${2 * brightnessFactor * glowIntensity}px rgba(${accentRGB}, ${0.1 * brightnessFactor * glowIntensity * lumaGlowFactor})
            `
            : `
    ...
    return (
        ...
        >
            {!isSource && !isCore && isWindowOpen && (
                <>
                        <div
                            className="absolute inset-0 -m-2 rounded-full border border-dashed pointer-events-none"
                            style={{ borderColor: selectionBorderColor }}
                        />
                </>
            )}
            ...
            {isSource && (
                <div className="relative flex items-center justify-center w-full h-full pointer-events-auto group">
                    ...
                    <div
                        className="absolute inset-0 rounded-full border animate-source-ping-large"
                        style={{ borderColor: isLuma ? 'rgba(42, 38, 32, 0.35)' : 'rgba(255, 255, 255, 0.2)' }}
                    />

                    {/* Central Dot (Smaller: 10px, Subtle Glow, Max Brightness) */}
                    <div
                        className="w-[10px] h-[10px] rounded-full animate-source-pulse pointer-events-none"
                        style={{
                            background: isLuma ? 'rgba(42, 38, 32, 0.8)' : '#ffffff',
                            boxShadow: isLuma
                                ? '0 0 8px rgba(42, 38, 32, 0.45)'
                    ...
                    <div
                        className="absolute top-[80px] text-[10px] tracking-[0.2em] font-mono pointer-events-none select-none"
                        style={{ color: isLuma ? 'rgba(42, 38, 32, 0.6)' : 'rgba(255, 255, 255, 0.5)' }}
                    >
                        SOURCE
                    </div>
                    ...
                    <div
                        className="absolute bottom-full mb-2 px-3 py-1.5 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-50"
                        style={{
                            background: isLuma ? 'rgba(233, 221, 198, 0.9)' : 'rgba(10, 10, 10, 0.7)',
                            border: isLuma ? '1px solid rgba(42, 38, 32, 0.15)' : '1px solid rgba(255, 255, 255, 0.1)',
                            color: isLuma ? 'rgba(42, 38, 32, 0.7)' : 'rgba(255, 255, 255, 0.7)'
                        }}
                    >
            ...
            {isCore && (
                <>
                    ...
                    <div className={clsx("absolute inset-0 flex items-center justify-center", shouldMaterialize && "animate-crystallize")}>
                        ...
                        <div
                            ...
                            style={{
                                ...
                                background: `conic-gradient(from 0deg,
                                    rgba(255, 0, 0, ${coreGlowAlpha}),
                                    rgba(255, 127, 0, ${coreGlowAlpha}),
                                    rgba(255, 255, 0, ${coreGlowAlpha}),
                                    rgba(0, 255, 0, ${coreGlowAlpha}),
                                    rgba(0, 255, 255, ${coreGlowAlpha}),
                                    rgba(0, 0, 255, ${coreGlowAlpha}),
                                    rgba(148, 0, 211, ${coreGlowAlpha}),
                                    rgba(255, 0, 0, ${coreGlowAlpha}))`,
                                filter: isLuma ? 'blur(18px)' : 'blur(20px)',
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{
                                    border: `1px solid ${selectionBorderStrong}`
                                }}
                            />
                        ...
                        <div>
                            ...
                            <span
                                className="relative z-10 block text-4xl font-sans leading-none select-none flex items-center justify-center w-[1em] h-[1em]"
                                style={{ filter: 'none', color: glyphColor }}
                            >
                                {glyphChar || '◉'}
                            </span>
                        </div>
            ...
            {isContainer && (
                <>
                    ...
                    {glyphChar && (
                        <span
                            className="absolute text-lg font-light leading-none select-none"
                            style={{ color: activeColor, textShadow: `0 0 10px rgba(${accentRGB}, 0.5)` }}
                        >
                            {glyphChar}
                        </span>
                    )}
                    ...
                    <div
                        className={clsx(
                            "absolute inset-0 rounded-full border transition-opacity scale-110",
                            isWindowOpen ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                        )}
                        style={{ borderColor: selectionBorderStrong }}
                    />
                </>
            )}
            ...
            {!isSource && !isCore && !isWindowOpen && (
                <div
                    className={clsx(
                        "absolute inset-0 -m-2 border border-dashed rounded-full transition-opacity pointer-events-none",
