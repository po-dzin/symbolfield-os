...
Supersedes: UI_PORTAL_TRANSITIONS_ANIMATION_SoT_v0.5_r3.md

# UI_PORTAL_TRANSITIONS_ANIMATION_SoT_v0.5_r3
**Scope:** SF v0.5 — spatial transitions across **Portal (Home/Station skin)** ↔ **Space Field** ↔ **NOW (node interior)**  
**Intent:** Make navigation feel like *moving in a place* (not switching tabs).  
**Canon:** `Enter` = Portal→Field · `NOW` = Portal/Field→Node interior · `Esc/Back` = NOW→Field
...
- **NOW is a lens** (deep view/editor for a node), presented as an overlay over Field.
...
- **ENTER (key / verb)**: **confirm + enter** the currently focused target.  
  ...
  - Node focused → enter **NOW** (node interior)
- **NOW (mode)**: the **Node interior / editor view** (deep focus). Triggered by Enter / Double‑click on a node, or via context menu.
...
| Field | NOW (Node) | Double click node / context → Open NOW | “Zoom into the node” | keeps spatial continuity |
...
- **Enter** (default)
- **NOW** (secondary)
- `…` (context: rename, pin, remove from recents)
